...
test("Agent can be passed in options", async (t) => {
	...
	const userAgent = getHeader(data.headers, "User-Agent");
...
test("Multiple requests can share the same Agent", async (t) => {
	...
	t.ok(
		getHeader(data1.headers, "User-Agent"),
		"First request should have User-Agent",
	);
	t.ok(
		getHeader(data2.headers, "User-Agent"),
		"Second request should have User-Agent",
	);
});
...
test("Agent can be used with other options", async (t) => {
	...
	const data = await response.json();
	t.ok(getHeader(data.headers, "User-Agent"), "Should have User-Agent");
	t.equal(
