...
test("credentials: default behavior is include", async (t) => {
  ...
  try {
    ...
    t.equal(response.status, 200, "should authenticate with default settings");
  } catch (error) {
...
test("credentials: 'include' preserves Cookie header in request", async (t) => {
  ...
  try {
    ...
    const data = await response.json();
    t.equal(data.cookies.test, "value", "should send first cookie");
    t.equal(data.cookies.another, "data", "should send second cookie");
  } catch (error) {
    t.fail(`Unexpected error: ${error.message}`);
  }
});
