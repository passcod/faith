...
test("Agent headers persist across multiple requests", async (t) => {
  ...
  t.equal(
    data1.headers["X-Persistent"],
    "persistent-value",
    "First request should have custom header",
  );
  t.equal(
    data2.headers["X-Persistent"],
    "persistent-value",
...
test("Agent headers without sensitive flag work", async (t) => {
  ...
  const agent = new Agent({
    headers: [{ name: "X-Public", value: "public-value", sensitive: false }],
  });
...
test("Multiple agents can have different headers", async (t) => {
  ...
  t.equal(
    data1.headers["X-Agent"],
    "agent1",
    "First agent should send its header",
  );
  t.equal(
    data2.headers["X-Agent"],
    "agent2",
...
test("Agent headers with multiple values", async (t) => {
  ...
  const data = await response.json();
  t.equal(data.headers["X-Header-1"], "value1", "First header should be set");
  t.equal(data.headers["X-Header-2"], "value2", "Second header should be set");
  t.equal(data.headers["X-Header-3"], "value3", "Third header should be set");
});
...
test("Agent headers work with other agent options", async (t) => {
  ...
  const data = await response.json();
  t.equal(data.headers["X-Custom"], "test", "Custom header should be sent");
  t.equal(
  ...
  t.ok(
    data.headers["X-Custom"] && data.headers["User-Agent"],
...
test("Agent headers with cookies option", async (t) => {
  ...
  const data1 = await response1.json();
  t.equal(data1.cookies.session, "test", "Cookie should be sent");

  const response2 = await faithFetch(headersUrl, { agent });
  const data2 = await response2.json();
  t.equal(
    data2.headers["X-Custom"],
    "with-cookies",
...
test("Invalid header names are silently omitted", async (t) => {
  ...
  const data = await response.json();
  t.equal(data.headers["Valid-Header"], "valid", "Valid header should be sent");
});
...
test("Agent headers with special characters in values", async (t) => {
  ...
  const agent = new Agent({
    headers: [
      { name: "X-Special", value: "value-with-dashes_and_underscores.dots" },
    ],
  });
...
test("Agent headers with empty value", async (t) => {
  ...
  const data = await response.json();
  t.equal(data.headers["X-Empty"], "", "Empty header value should be sent");
});
...
test("Agent headers work with parallel requests", async (t) => {
  ...
  t.equal(
    data1.headers["X-Parallel"],
    "parallel-value",
    "First request should have custom header",
  );
  t.equal(
    data2.headers["X-Parallel"],
    "parallel-value",
    "Second request should have custom header",
  );
...
test("Agent headers with case variations", async (t) => {
  ...
  const data = await response.json();
  t.ok(
    data.headers["x-lowercase"] || data.headers["X-Lowercase"],
    "Lowercase header should be sent",
  );
  t.ok(
    data.headers["X-UPPERCASE"] || data.headers["X-Uppercase"],
    "Uppercase header should be sent",
  );
  t.ok(
    data.headers["X-MixedCase"] || data.headers["X-Mixedcase"],
...
test("Agent headers combined with request headers", async (t) => {
  ...
  const data = await response.json();
  t.equal(
    data.headers["X-Agent-Header"],
    "from-agent",
    "Agent header should be sent",
  );
  t.equal(
    data.headers["X-Request-Header"],
    "from-request",
