...
test("badssl.com - peer.certificate is available for HTTPS", async (t) => {
	t.plan(5);

	const response = await faithFetch("https://badssl.com/");

	t.ok(response.peer, "peer should exist");
	t.ok(response.peer.certificate, "peer.certificate should exist for HTTPS");
	t.ok(
		Buffer.isBuffer(response.peer.certificate),
		"peer.certificate should be a Buffer",
	);
	t.ok(
		response.peer.certificate.length > 0,
		"peer.certificate should not be empty",
	);
	t.ok(response.peer.address, "peer.address should also be present");
});

test("badssl.com - peer.certificate is DER-encoded", async (t) => {
	t.plan(3);

	const response = await faithFetch("https://badssl.com/");
	const cert = response.peer.certificate;

	t.ok(cert, "certificate should exist");
	t.equal(
		cert[0],
		0x30,
		"DER certificate should start with SEQUENCE tag (0x30)",
	);
	t.ok(cert.length > 100, "DER certificate should be reasonably sized");
});

test("badssl.com - peer.certificate persists after reading body", async (t) => {
	t.plan(3);

	const response = await faithFetch("https://badssl.com/");
	await response.text();

	t.ok(response.peer, "peer should still exist after reading body");
	t.ok(
		response.peer.certificate,
		"peer.certificate should still exist after reading body",
	);
	t.ok(
		Buffer.isBuffer(response.peer.certificate),
		"peer.certificate should still be a Buffer",
	);
});

test("badssl.com - different HTTPS hosts have different certificates", async (t) => {
	t.plan(3);

	const response1 = await faithFetch("https://sha256.badssl.com/");
	const response2 = await faithFetch("https://ecc256.badssl.com/");

	t.ok(response1.peer.certificate, "First host should have certificate");
	t.ok(response2.peer.certificate, "Second host should have certificate");
	t.notDeepEqual(
		response1.peer.certificate,
		response2.peer.certificate,
		"Different hosts should have different certificates",
	);
});

test("badssl.com - peer.certificate available for various cert types", async (t) => {
	t.plan(8);
	...
	for (const host of hosts) {
		const response = await faithFetch(host);
		t.ok(response.peer.certificate, `${host} should have certificate`);
		t.ok(
			Buffer.isBuffer(response.peer.certificate),
			`${host} certificate should be Buffer`,
		);
	}
});
