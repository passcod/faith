...
test("Agent with cookies enabled can add and retrieve cookies", async (t) => {
  ...
  t.equal(data.cookies.session, "abc123", "Server should receive the cookie");
});
...
test("Multiple cookies can be added to an agent", async (t) => {
  ...
  const data = await response.json();
  t.equal(data.cookies.session, "abc123", "First cookie should be sent");
  t.equal(data.cookies.user, "john", "Second cookie should be sent");
  t.equal(data.cookies.theme, "dark", "Third cookie should be sent");
});
...
test("Server-set cookies are stored by agent", async (t) => {
  ...
  t.equal(data.cookies.name, "value", "Stored cookie should be sent");
});
...
test("Different agents have separate cookie stores", async (t) => {
  ...
  t.equal(
    data1.cookies.session,
    "agent1",
    "First agent should send its cookie",
  );
  t.equal(
    data2.cookies.session,
    "agent2",
    "Second agent should send its cookie",
  );
  t.notEqual(
    data1.cookies.session,
    data2.cookies.session,
...
test("Cookies with attributes are handled correctly", async (t) => {
  ...
  t.equal(
    ...
    "abc123",
    "Cookie value should be sent correctly",
  );
});
...
test("Adding cookie overwrites previous value", async (t) => {
  ...
  const data = await response.json();
  t.equal(data.cookies.session, "new", "Cookie should have the updated value");
});
...
test("Parallel requests with cookies work correctly", async (t) => {
  ...
  t.equal(
    data1.cookies.session,
    "parallel",
    "First request should have cookie",
  );
  t.equal(
    data2.cookies.session,
    "parallel",
  ...
  t.equal(
    data3.cookies.session,
    "parallel",
    "Third request should have cookie",
  );
});
...
test("Complex cookie value with special characters", async (t) => {
  ...
  const data = await response.json();
  t.ok(data.cookies.data, "Cookie with encoded value should be sent");
});
