...
With `credentials: "omit"` on a request, `Cookie` is not sent and `Set-Cookie` is stripped from the returned headers; that the jar still ingests those `Set-Cookie` values is a known upstream limitation, documented rather than hidden (see [REQ](../fetch/request.md)).
