...
Two agent methods, carrying the same names on both surfaces, let a caller pay connection-setup costs before the first request needs them, so that request skips the round trips.
DNS prefetch warms the DNS cache; `preconnect(origin)` resolves the host and opens a pooled connection.
...
They differ in what they touch: DNS prefetch reaches only the resolver, while `preconnect` reaches the origin itself with a synthetic request, which the origin can see.
DNS prefetch cannot change the outcome of a later request, only its speed.
...
DNS prefetch resolves a host through the agent's resolver and stores the answer in the in-memory DNS cache, so a later request to that host skips the lookup (see [DNS](dns.md)).
A DNS name has no scheme, port, or path, so those parts are ignored if a fuller string is passed.
Resolution honours the agent's DNS overrides and races IPv4 and IPv6 under Happy Eyeballs exactly as a request's own lookup does.
With the system resolver there is no cache to warm, so the call resolves successfully without doing any work.
...
Resolving the host warms the DNS cache as a side effect, so `preconnect` subsumes DNS prefetch for that host; under the system resolver there is no cache and the lookup only serves the connection.
...
Turning HTTP/3 upgrade probing off does not suppress it either: that option governs HTTP/3 upgrade probing and continues to do only that, so a TCP warm-up still sends its `HEAD` (see [PROBE](../http3/probing.md)).
...
It otherwise carries the agent's configuration like any request to that origin, including default headers, the `User-Agent`, the cookie jar, and the agent's redirect handling, so an origin that sets cookies on its root can update the jar from a warm-up (see [COOK](cookies.md)).
...
A warm-up connection is an ordinary idle pooled connection: the pool's idle timeout closes it when no request claims it in time, and the per-host idle cap bounds it like any other (see [POOL](connection-pool.md)).
...
A request that lands on a TCP warm-up connection is reported as reused in its timing breakdown, which is how a caller confirms a warm-up was actually spent rather than having lapsed (see [RESP](../response/response.md)).
...
A `preconnect` moves the background-requests counter instead, which is where the agent's self-initiated wire traffic is counted; DNS prefetch reaches only the resolver and so moves nothing (see [OBS](observability.md)).
They coalesce with work already in progress: a `preconnect` for an origin that already holds an idle pooled connection, or a DNS prefetch for a host already fresh in the cache, does no new work, and concurrent warm-ups for the same target are single-flighted rather than opening duplicates.
