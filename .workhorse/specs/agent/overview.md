...
Errors that indicate a broken configuration throw: an unparseable `localAddress` or DNS override address (syntax error), malformed `tls.identity` or `tls.extraRoots` PEM (syntax error), and a disk cache without a path (configuration error).
...
Nested option groups are specified in their own areas: [POOL](connection-pool.md), [COOK](cookies.md), [DNS](dns.md), [TLS](tls.md), [OBS](observability.md), [H2](http2.md), [CACHE](../cache/http-cache.md), [H3UP](../http3/upgrade.md), [QUIC](../http3/transport.md), and [REDIR](../fetch/redirects.md) and [CANCEL](../fetch/cancellation-and-timeouts.md) for the `redirect` and `timeout` groups.
