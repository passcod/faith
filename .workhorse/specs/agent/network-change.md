...
- [ ] The resolver drops everything it read off the network: the cached answers, the servers discovery took from the system, the suffixes it treats as local, and which servers answered an encryption probe. Names then resolve afresh through the servers the new network supplies, rather than being looked up again through the previous network's. Under the system resolver there is no resolver of Faith's and the signal does no DNS work (see [DNS](dns.md)).
