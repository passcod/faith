...
Network-shaped failures throw an error named `NetworkError`: `Network` covers transport and protocol failures, `Redirect` a redirect refused by an agent whose redirect policy is `error` (see [REDIR](../fetch/redirects.md)), and `ContentLengthOverrun` a response body exceeding the length the server advertised (see [BODY](../response/reading-the-body.md)).
