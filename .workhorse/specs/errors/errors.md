...
Malformed input that has a textual syntax throws `SyntaxError`: `AddressParse` (DNS override, DNS server URL, or local address), `InvalidIntegrity` (SRI value), `JsonParse` (`response.json()`), `PemParse` (TLS identity or extra roots).
API misuse throws `TypeError`: `InvalidHeader`, `InvalidMethod`, `MissingContentType` (a `QUERY` request carrying a body no `Content-Type` describes, see [REQ](../fetch/request.md)), `InvalidUrl`, `InvalidCompression` (a `compress` option naming no coding Faith can compress in, see [ENC](../fetch/content-encoding.md)), `Closed` (request on a closed agent), `ResponseAlreadyDisturbed` (reading a body that has been consumed or discarded, see [BODY](../response/reading-the-body.md)), `ResponseBodyNull` (writing a response that cannot carry a body to a file), and `InvalidPath` (a file destination that does not name a local path).
