...
API misuse throws `TypeError`: `InvalidHeader`, `InvalidMethod`, `InvalidUrl`, `Closed` (request on a closed agent), `ResponseAlreadyDisturbed` (reading a body that has been consumed or discarded, see [BODY](../response/reading-the-body.md)), `ResponseBodyNull` (writing a response that cannot carry a body to a file), and `InvalidPath` (a file destination that does not name a local path).
