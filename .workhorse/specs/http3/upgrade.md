...
## Advertisements from DNS

An `HTTPS` DNS record (RFC 9460) is a second source of advertisement alongside the `Alt-Svc` header, read while the name is resolved rather than from a response, so an origin's HTTP/3 support is learnt before the first connection instead of from the first TCP response's header (see [DNS](../agent/dns.md)).
