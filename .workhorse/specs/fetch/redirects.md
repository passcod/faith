...
`follow` (the default) follows redirects automatically, up to 10 hops; exceeding the limit is a network error carrying the `Network` code.
`error` rejects the fetch promise with a network error carrying the `Redirect` code when a redirect status is returned (see [ERR](../errors/errors.md)).
