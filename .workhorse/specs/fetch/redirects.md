...
With the agent's advertised-port quirk enabled, URL comparison for this flag ignores the port (the port is rewritten by the upgrade), so a genuine redirect differing only in port reads as not-redirected on those responses (see [H3UP](../http3/upgrade.md)).
