...
The cache serve-stale option governs whether the agent honours that opt-in, defaulting to on so a compliant origin's directives work without configuration.
Turned off, both directives are ignored: a stale match is revalidated in the foreground and a failed revalidation surfaces to the caller, which is the behaviour of an agent that never serves a body it knows to be out of date.
