...
Fáith implements the semantics of the fetch specification's trailers proposal (whatwg/fetch#1940).
...
Awaiting the trailers without anything consuming the body waits forever; this is the proposal's specified behaviour, not a deadlock bug.
...
Trailers are supported on HTTP/1.1 (chunked), HTTP/2, and HTTP/3 responses, as exercised by the conformance matrix (see [CONF](../quality/conformance.md)).
